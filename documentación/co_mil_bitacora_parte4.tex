...
\textbf{Resultado} (corrida en GPU T4, 3 semillas, 6 de septiembre; Figura~\ref{fig:curva}). Además
de las curvas \texttt{mixto} y \texttt{solo\_supervisado}, se probó la idea del autor de \emph{filtrar}
la anotación débil a las imágenes con $\leq 1$ o $\leq 2$ tejidos presentes (opción
...
3 semillas. \texttt{solo}: $N$ máscaras y nada más. \texttt{mixto}: $N$ máscaras $+$ las $78-N$
...
\caption{Frontera de eficiencia de anotación en DFUTissue (3 semillas, banda $= \pm 1\sigma$). Las
...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{La anotación débil no aporta nada sobre las máscaras densas a esta escala.} Las
        curvas \texttt{mixto} y \texttt{solo} coinciden dentro del ruido en todo el rango (en $N=40$
        la mixta es incluso levemente peor). Concuerda con la literatura y con la predicción previa:
    ...
    \item \textbf{Varianza alta a $N$ bajo} (desviación 0.06--0.19). Tres semillas es el mínimo; se
        repetirá con cinco antes de la versión final del capítulo.
\end{itemize}
...
Drive y reanudan. La primera corrida (bloques A/B/C, semillas 42/1/7) se ejecutó en segundo plano sin
incidencias; queda un bloque adicional (semillas 2 y 3) para pasar a cinco semillas antes de fijar
