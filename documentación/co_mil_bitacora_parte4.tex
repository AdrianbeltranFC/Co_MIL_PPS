...
por tanto, íntegramente computacional sobre el conjunto de datos público DFUTissue, preparando el
estudio de eficiencia para poder ejecutarlo en GPU en cuanto haya tiempo de máquina.
...
se filtran. Se comprobó el cableado con una corrida corta y se verificó el conteo real de imágenes
débiles resultante.
...
El experimento 1 pasa a comparar \textbf{cuatro curvas en una sola corrida}: \texttt{solo\_
supervisado} ($N$ máscaras y nada más), \texttt{mixto} ($N$ máscaras + todas las etiquetas débiles),
y \texttt{mixto $\leq 1$} / \texttt{mixto $\leq 2$} (con el filtro anterior). La brecha
\texttt{mixto} $-$ \texttt{solo\_supervisado} mide el valor real de la anotación débil; la brecha
\texttt{mixto} $-$ \texttt{mixto $\leq k$} mide si conviene filtrarla.

El cuaderno se dividió en bloques (uno por semilla para el experimento 1, más un bloque para los
experimentos 2 y 3) para que cada uno quepa en una sesión de Colab gratuita ($\sim$75--90\,min en
T4). El script gana la opción \texttt{--dir\_salida}, que reutiliza una carpeta de resultados y
\emph{reanuda} el barrido saltando las corridas ya completadas: los tres bloques de semilla escriben
en la misma carpeta de Google Drive y, si una sesión se corta, basta relanzar la misma celda.
