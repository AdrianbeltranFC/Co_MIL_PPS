...
imágenes de entrenamiento de DFUTissue; 122 épocas, $\sim$46\,min en CPU. Resultado sobre las 16
imágenes de prueba oficiales:
...
\texttt{fig\_debil\_vs\_fuerte}, \texttt{fig\_flujo\_anotacion}. Pendiente: actualizar el diagrama de
arquitectura del modelo (\texttt{fig\_arquitectura\_modelo}, hoy muestra la red MIML de $K$ ramas) y
el del pipeline (\texttt{fig\_pipeline\_estado\_actual}) para reflejar los tres brazos y la anotación
densa. El diagrama del mecanismo de atención (\texttt{fig\_mecanismo\_atencion\_mil}) se conserva:
sigue siendo válido para describir el brazo débil.
